...
public class Kalman : MonoBehaviour
{
    ...
    private float unitTorque = 8; // torque per 1 m/s
    private float manualSpeed = 2; // WSAD move speed; m/s
    private float manualSteerAngle = 30 * Mathf.Deg2Rad; // WSAD steer angle; radians
    ...
    private async void Start()
    {
        ...
        // Initialize PIDs.
        flTurnPid = frTurnPid = blTurnPid = brTurnPid = new PID(0.1F, 0, 0);
    ...
    {
        flWheel.steerAngle += flTurnPid.Update(flWheel.steerAngle, -wheelStates.FrontLeft.Angle * Mathf.Rad2Deg, Time.fixedDeltaTime);
        frWheel.steerAngle += frTurnPid.Update(frWheel.steerAngle, -wheelStates.FrontRight.Angle * Mathf.Rad2Deg, Time.fixedDeltaTime);
        blWheel.steerAngle += blTurnPid.Update(blWheel.steerAngle, -wheelStates.BackLeft.Angle * Mathf.Rad2Deg, Time.fixedDeltaTime);
        brWheel.steerAngle += brTurnPid.Update(brWheel.steerAngle, -wheelStates.BackRight.Angle * Mathf.Rad2Deg, Time.fixedDeltaTime);
