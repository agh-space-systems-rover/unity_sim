...
void handle_client(int client_socket, std::string client_id, size_t data_size)
{
  ...
  rclcpp::Publisher<sensor_msgs::msg::CameraInfo>::SharedPtr aligned_depth_to_color_info_pub,
    color_info_pub;
  ...
  std::optional<unity_rs_publisher_msgs::msg::CameraMetadata> meta;
  {
    ...
    RCLCPP_INFO(
      ros_node->get_logger(), "Client %s connected. Frame data size: %d", client_id.c_str(),
      static_cast<int>(data_size));

    // Create a publishers for this client.
    // See: https://github.com/IntelRealSense/realsense-ros
    //
    aligned_depth_to_color_info_pub = ros_node->create_publisher<sensor_msgs::msg::CameraInfo>(
      "/" + client_id + "/aligned_depth_to_color/camera_info", 1);
    aligned_depth_to_color_pub =
      ros_it->advertise("/" + client_id + "/aligned_depth_to_color/image_raw", 1);
    color_info_pub = ros_node->create_publisher<sensor_msgs::msg::CameraInfo>(
    ...
    meta_sub = ros_node->create_subscription<unity_rs_publisher_msgs::msg::CameraMetadata>(
      "/" + client_id + "/unity_rs_publisher/meta", 1,
      [&meta](const unity_rs_publisher_msgs::msg::CameraMetadata::SharedPtr msg) { meta = *msg; });
  ...
  while (true) {
    ...
    int bytes_received =
      recv(client_socket, data.data() + data_received, data_size - data_received, 0);
    if (bytes_received <= 0) {
    ...
    if (data_received == data_size) {
      ...
      point_cloud.data.reserve(
        point_cloud_width * point_cloud_height * 4 *
        4);  // 4 bytes per float; 4 floats per point (x, y, z, rgb); rgb is
             // actually uint32
      point_cloud.data.clear();
      float subsample_radius = static_cast<float>(height) / point_cloud_height / 2;
      for (size_t row = 0; row < point_cloud_height; row++) {
        for (size_t col = 0; col < point_cloud_width; col++) {
          ...
          // Subsample points.
          src_row = static_cast<float>(src_row) + dist(rng) * subsample_radius;
          src_col = static_cast<float>(src_col) + dist(rng) * subsample_radius;
      ...
      camera_info.k = {meta->fx, 0, meta->cx, 0, meta->fy, meta->cy, 0, 0, 1};
      camera_info.r = {1, 0, 0, 0, 1, 0, 0, 0, 1};
      camera_info.p = {meta->fx, 0, meta->cx, 0, 0, meta->fy, meta->cy, 0, 0, 0, 1, 0};
...
int main(int argc, char ** argv)
{
  ...
  while (true) {
    ...
    int bytes_received = recv(
      client_socket, data_size_plus_client_id_buffer, sizeof(data_size_plus_client_id_buffer), 0);
    if (bytes_received <= 0) {
      close(client_socket);
      continue;
    }
